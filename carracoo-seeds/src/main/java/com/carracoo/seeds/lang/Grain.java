...
import com.carracoo.utils.ReflectUtils;
	...
	public Grain(){
		try {
			...
			if(Corn.class.isAssignableFrom(p.getType())){
				...
				parrent		= (P) p.get(this);
				if(!this.getClass().getEnclosingClass().equals(parrent.clazz())){
					throw new RuntimeException("Grain definition should be in parrent class <"+this.getClass().getEnclosingClass()+","+parrent.clazz()+">");
				}	
				type = ReflectUtils.getTypeArguments(Grain.class, (Class<Grain>)getClass()).get(1);
				Corn.FieldInfo f = parrent.field();
				if(f!=null){
